...
告别逐台登录 Web 界面手动升级的繁琐操作——只需粘贴一批 IP 地址、选择固件文件（`.exob`）、点击开始，工具即可自动完成所有目标设备的认证、上传、解压和重启操作，支持多设备并行处理。
...
3. **准备环境** — 重置设备上的工程环境
4. **上传固件** — 以 multipart 表单形式 POST 上传 `.exob` 固件文件
